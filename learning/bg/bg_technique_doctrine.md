# 背景クオリティ・ドクトリン（蒸留版 v6）

動画 26 本+公式 doc 4 本の横断抽出（v6: 「Secrets behind GREAT Looking Environments」統合＝補色ペア／ネガティブスペース／シルエット破線化）。**日常はこれだけ読む**（上限 3.5KB 目安）。詳細は `videos/`（Sonnet 委譲）。
...
5. **構図は F/M/B + フォーカルポイント** — 全リーディングラインを焦点へ収束、「どこからでも見える」を保証。コントラスト最大は焦点側のみ（素材の質感差=有機物vs硬質面でも作れる）。**補色ペア**（青×橙／緑×赤／橙×紫）は鉄板配色、色相環上の距離＝コントラスト量。**ネガティブスペースを残す**（全面を埋めない）。**シルエットは小要素で輪郭を崩す**と印象が変わる
...
- **PLI 量産**: 使い回す群を選択→Packed Level Actor 化（Pivot=MinZ 接地/Center。⚠事後変更不可）。Edit→Commit で全インスタンス反映。複製は回転+スケールでシルエット差し（一様 1.3〜1.7・非一様 1.75 倍までバレない［伝聞］）
